...
const JobSection = () => {
  ...
  // Suppression de la variable scrollPosition qui n'est pas utilisée
  ...
  const handleScroll = () => {
    if (carouselRef.current) {
      ...
      // Suppression de la ligne qui définit scrollPosition
        ...
        <div className="jobs-carousel" ref={carouselRef}>
          {jobs.map((job) => (
            <div key={job.id} className="job-card">
                ...
                {job.companyLogo ? (
                  <div className="job-logo">
                    <img src={job.companyLogo} alt={job.company} onError={(e) => {
                      ...
                      // Utiliser les initiales de l'entreprise au lieu de 'NA'
                      const companyName = job.company || 'Entreprise non spécifiée';
              ...
              <div>
                <h3 className="job-title">{job.title || 'Poste non spécifié'}</h3>
                <p className="job-company">{job.company || 'Entreprise non spécifiée'}</p>
                
                <div className="job-meta">
                  <span className="job-location">{job.location || 'Lieu non spécifié'}</span>
                  <span className="job-contract">{job.contractType || 'Type non spécifié'}</span>
